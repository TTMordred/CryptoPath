    ...
    <div className="w-full max-w-3xl mx-auto animate-slide-up">
      <div className="glass-card rounded-xl p-6 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
        <div className="mb-8 flex items-center justify-between">
          ...
          {!isAdding && (
            <Button 
              className="bg-amber text-black hover:bg-amber-light transition-all duration-300 font-medium rounded-full shadow-[0_2px_10px_rgba(246,179,85,0.2)]"
          ...
          <div className="neo-blur p-5 rounded-xl mb-8 animate-fade-in">
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex space-x-3">
                <Button 
                  className="bg-amber text-black hover:bg-amber-light transition-all duration-300 rounded-full flex-1"
                ...
                <Button 
                  variant="outline"
                  className="border-white/20 text-white/80 hover:bg-white/5 rounded-full flex-1"
                  onClick={() => {
                    setIsAdding(false);
                    setNewWalletAddress('');
                  }}
            ...
            wallets.map((wallet) => (
              <div 
                ...
                className={`neo-blur p-4 rounded-xl flex items-center justify-between transition-all duration-300 ${
                  wallet.isDefault ? 'border-amber shadow-[0_0_10px_rgba(246,179,85,0.2)]' : 'border-white/10'
                } hover:border-white/20`}
              >
                <div className="flex items-center space-x-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                    wallet.isDefault ? 'bg-amber text-black' : 'bg-black/70 text-white/80'
                  }`}>
