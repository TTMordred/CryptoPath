  ...
  return (
    <div className="backdrop-blur-md bg-shark-700/30 border border-shark-600 rounded-2xl shadow-lg transition-all duration-300 hover:border-amber/30 p-6 h-full opacity-0 animate-[fadeIn_0.5s_ease-out_forwards]">
      ...
      <div className="space-y-4 min-h-[300px]">
        {isLoading ? (
          [...Array(2)].map((_, i) => (
            <div key={i} className="flex p-3 border border-shark-600 rounded-lg">
              <Skeleton className="h-24 w-24 rounded-md bg-shark-600/50" />
              <div className="ml-3 flex-1">
                <Skeleton className="h-4 w-3/4 bg-shark-600/50 mb-2" />
                <Skeleton className="h-3 w-1/2 bg-shark-600/50 mb-3" />
                <Skeleton className="h-3 w-full bg-shark-600/50" />
                <Skeleton className="h-3 w-5/6 bg-shark-600/50 mt-1" />
              </div>
            </div>
          ))
        ) : nfts.length > 0 ? (
          currentNFTs.map((nft, index) => (
            ...
            >
              ...
              <div className="ml-3 flex-1">
                <h4 className="text-gray-200 font-medium overflow-hidden text-ellipsis whitespace-nowrap">{nft.name || `#${nft.tokenId}`}</h4>
                <p className="text-amber text-xs mb-2">{nft.collectionName}</p>
                <p className="text-gray-400 text-xs overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
