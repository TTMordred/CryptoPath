    ...
    const fetchWalletAddress = async () => {
      ...
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from("profiles")
          .select("wallets") // Truy vấn cột wallets kiểu JSON
          .eq("id", userId)
          .single();
        ...
        if (!profileData.wallets || !Array.isArray(profileData.wallets)) {
          console.warn("No valid wallets found, redirecting to settings.");
          router.push("/settings");
          return;
        }

        const defaultWallet = profileData.wallets.find((wallet) => wallet.is_default);
      ...
      {walletAddress && (
        <main className="px-6 mx-auto max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <BalanceCard balance={walletData?.balance || "0"} isLoading={isLoading} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
              <AllocationChart
                ...
                ethBalance={walletData?.balance || "0"}
                isLoading={isLoading}
              />
            </div>
            <div>
